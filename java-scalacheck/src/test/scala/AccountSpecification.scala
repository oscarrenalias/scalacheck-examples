...
object AccountSpecification extends Properties("Account") {
    ...
    property("Deposit") = forAll(genAcctAmt) { case (acct: Account, amt: Double) =>
        val oldBalance = acct.getBalance()
        acct.deposit(amt)
        acct.getBalance() == oldBalance + amt
    }
    
    property("Withdraw-normal") = forAll(genAcctAmt) { case (acct: Account, amt: Double) =>
        amt <= acct.getBalance() ==> {
            val oldBalance = acct.getBalance()
            acct.withdraw(amt)
            acct.getBalance() == oldBalance - amt
    ...
    property("Rate-lowBalance, lowAge") = {
        ...
        forAll(gen) { acct: Account => acct.getRate() == STD_INTEREST }
    }
