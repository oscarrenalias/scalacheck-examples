...
object DataCollectionSpecification extends Properties("Data collection examples") {
	...
	/**
	 * Here we use the ÒbinaryÓ version of Prop.classify to classify our random rectangle objects into ÒtallerÓ or ÒwiderÓ,
	 * and then with Prop.collect weÕve collected the data using our previous method. The output now is a two-level
